from management.ai_bases import async_get_response_ai_1_large, async_get_response_ai_2_ultimate #get_response_ai_1, get_response_ai_2
...
async def ai_processing(prompt, files, components, chat, is_first_prompt, technology, attempt):
    ...
    try:
        chat_category = await sync_to_async(ChatCategory.objects.get)(type='supreme')
        ...
        if is_first_prompt:
            ...
            ai_answer_1 = await async_get_response_ai_1_large(engineered_prompt_1, chat)
            ...
            await sync_to_async(CodingChatMessage.objects.create)(
                chat=chat, type='ai', content=ai_answer_1, order=max_order+1, api_key=None,
            ...
            ai_answer_2 = await async_get_response_ai_2_ultimate(engineered_prompt_2, chat)
            #print('AI ANSWER 2!!!!!!')
            #print(ai_answer_2)
            max_order = await sync_to_async(
                lambda: CodingChatMessage.objects.filter(chat=chat).aggregate(Max('order'))['order__max'])()
            ...
            ai_adjustment_answer = await async_get_response_ai_2_ultimate(engineered_adjustment_prompt, chat)
...
async def save_prompt(chat, prompt, chat_category_id, processing_steps, attempt_number):
    ...
    max_order = await sync_to_async(lambda: CodingChatMessage.objects.filter(chat=chat).aggregate(Max('order'))['order__max'])() or 0
...
async def get_related_files(engineered_prompt_0, files, chat, chat_category_id, processing_steps, attempt_number):
    ...
    max_order = await sync_to_async(lambda: CodingChatMessage.objects.filter(chat=chat).aggregate(Max('order'))['order__max'])()
    try:
        ai_response = await async_get_response_ai_1_large(engineered_prompt_0, chat)
...
async def get_related_components(engineered_prompt_1, components, chat, chat_category_id, processing_steps, attempt_number):
    try:
        max_order = await sync_to_async(lambda: CodingChatMessage.objects.filter(chat=chat).aggregate(Max('order'))['order__max'])()
        ai_response = await async_get_response_ai_1_large(engineered_prompt_1, chat)
...
async def get_final_solution(engineered_prompt_2, chat, chat_category_id, processing_steps, attempt_number):
    ...
    if engineered_prompt_2 == "None":
        ...
        ai_response = await async_get_response_ai_2_ultimate(engineered_prompt_2, chat)
        await sync_to_async(CodingChatMessage.objects.create)(
            chat=chat,
            type='gpt-a',
            content=ai_response,
...
async def get_adjusted_solution(chat, engineered_adjustment_prompt, chat_category_id, processing_steps, attempt_number):
    # appel à l'API IA
    ai_response = await async_get_response_ai_2_ultimate(engineered_adjustment_prompt, chat)
    max_order = await sync_to_async(
