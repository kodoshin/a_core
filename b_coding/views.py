...
def delete_group_messages(request):
    ...
    msgs = list(CodingChatMessage.objects.filter(chat=chat, attempt_number=attempt_no).order_by('id'))
    ...
    qs = CodingChatMessage.objects.filter(chat=chat, attempt_number=attempt_no, id__gte=prompt_msg_id)
    if next_prompt:
