...
async def planning_chat_view(request):
    user = request.user
    ...
    if request.method == 'POST':
        ...
        if 'regenerate' in request.POST:
            ...
            try:
                ...
                chat = await sync_to_async(PlanningChat.objects.select_related('chat_category').get)(public_id=chat_id,
                                                                                                   user=user)
        ...
        if 'prompt' in request.POST and 'regenerate' not in request.POST:
            ...
            #print(attempt_no)
            if chat_id:
                chat = await sync_to_async(PlanningChat.objects.get)(public_id=chat_id, user=user)
                default_chat_category = await sync_to_async(lambda: chat.chat_category)()
                is_first_prompt = False
            else:
                chat = await sync_to_async(PlanningChat.objects.create)(user=user, project=default_project)
            ...
            if available_credits >= default_chat_category.price:
                ...
                if default_chat_category.type == 'regular':
                    #response_task = asyncio.create_task(regular_ai_processing(prompt, components, chat, is_first_prompt, technology, attempt_no))
                    pass
                elif default_chat_category.type == 'super':
                    #response_task = asyncio.create_task(super_ai_processing(prompt, files, components, chat, is_first_prompt, technology, attempt_no))
                    pass


                # Await the AI response
                ai_response = 'MODIFY:To activate response_task' #await response_task
                ...
                steps = 'MODIFY:To create parse_steps_json' #parse_steps(fix_response_format(ai_response))
                return JsonResponse({
                    'user_message': prompt,
                ...
                processing_steps = await sync_to_async(lambda: {f'{step.order} : {step.name}': step
                                                                for step in PlanningProcessingStep.objects.all()})()
                if chat.title is None:
                    chat.title = prompt[:25] + '...' if len(prompt) >= 28 else prompt
                    await sync_to_async(chat.save)()
                await sync_to_async(PlanningChatMessage.objects.create)(
        ...
        chats = await sync_to_async(lambda: list(PlanningChat.objects.filter(user=user, project=default_project).order_by('-created_on')))()
        chat_id = request.GET.get('chat_id')
        if chat_id:
            # On récupère le chat et le nombre total d'essais
            try :
                current_chat = await sync_to_async(lambda: PlanningChat.objects.select_related('chat_category').get(public_id=chat_id, user=user))()
            ...
            has_ai = await sync_to_async(lambda: current_chat.messages
                                         ...
                                         .exists())()
            has_error = await sync_to_async(lambda: PlanningProcessingError.objects
            ...
            for msg in raw_messages:
                ...
                if msg.type == 'gpt-a':
                    msg_dict['steps'] = 'MODIFY:To create parse_steps_json' #parse_steps(fix_response_format(msg.content))
                else:
            ...
            total_attempts = 1
            selected_attempt = 1
        chatcategories = await sync_to_async(lambda: list(PlanningChatCategory.objects.filter(is_active=True).order_by('price')))()
        context = {
            'chats': chats,
            'messages': messages,
